package org.websocket.stomp.websocket;
...
public final class StompWebSocketClientPageHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest request) {
        ...
        if (request.headers().contains(HttpHeaderNames.UPGRADE, HttpHeaderValues.WEBSOCKET, true)) {
            System.out.println("channelRead0 : Received request " + request.uri());
            ctx.fireChannelRead(request.retain());
            return;
        }

        if (request.decoderResult().isFailure()) {
            System.out.println("channelRead0 : isFailure " + request.uri());
        ...
        if (!sendResource(request, ctx)) {
            ...
            System.out.println("channelRead0 : sendResource " + notFound);
    ...
    private static boolean sendResource(FullHttpRequest request, ChannelHandlerContext ctx) {
        ...
        System.out.println("[sendResource url] : " + resourceUrl);
        System.out.println("[sendResource] : " + requestResource);
        ...
        try {
            ...
            System.out.println("[sendResource] " + raf);
